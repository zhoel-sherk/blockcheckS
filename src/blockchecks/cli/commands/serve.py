...
Unix-socket core (asyncio.start_unix_server, no deps) + optional thin HTTP
bridge on 127.0.0.1. Fair exclusion with long-term campaigns: while a campaign
owns run.lock, every probe request returns busy/campaign_active (423-style)
instead of blocking.
"""
            ...
            try:
                ...
                if http_port:
                    await asyncio.gather(
                        server.serve(),
                        server.serve_http(port=int(http_port)),
                    )
                else:
                    await server.serve()
            finally:
