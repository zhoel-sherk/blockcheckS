...
def cmd_udp(args):
        ...
        try:
            ...
            if multi_eps:
                ...
                for ep in multi_eps[1:3]:
                    log.info(
                        "%s",
                        f"  {GREEN}  + {ep['ip']}:{ep['port']} ({ep.get('hostname', '')}){RESET}",
                    )
                if len(multi_eps) > 3:
                    log.info("%s", f"  {GREEN}  ... and {len(multi_eps) - 3} more{RESET}")
            else:
                log.info(
                    "%s", f"  {YELLOW}No alive endpoints — using static DEFAULT_VOICE_*{RESET}"
                )
    ...
    log.info("%s", f"  Targets: {len(targets)}  Items: {len(configs)}  Timeout: {args.timeout}s\n")
    runner = TestRunner(ns_name=args.ns)
    passed_any = 0
    total_time = 0.0
    for ip, port in targets:
        log.info("%s", f"  {CYAN}--- ep={ip}:{port} ---{RESET}")
