...
def run_probe(ip: str, port: int, timeout: float) -> dict:
...
def main(argv: list[str] | None = None) -> int:
    ...
    if len(args) != 3:
        print("usage: python -m blockchecks.engine._probe_worker IP PORT TIMEOUT", file=sys.stderr)
        return 2
    ip, port_s, timeout_s = args
    data = run_probe(ip, int(port_s), float(timeout_s))
