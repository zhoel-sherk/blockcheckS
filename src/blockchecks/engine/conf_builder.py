...
def _export_part(part: str) -> list[str]:
    ...
    if part.startswith("--"):
        cores = [
            ...
            if tok.startswith("--lua-desync=") and is_lua_function_core(tok[len("--lua-desync=") :])
        ]
        if cores:
            return cores
        if "/" in part:
...
def strategy_parts_for_export(strat: str) -> list[str]:
    ...
    return [p for raw in text.split("\n") if (part := raw.strip()) for p in _export_part(part)]


def flatten_export_strategies(strategies: list[str]) -> list[str]:
    """Function cores only (no ``--`` CLI fragments)."""
    return [p for s in strategies for p in strategy_parts_for_export(s) if not p.startswith("--")]
...
def _keenetic_copy_header(
    ...
    return [
        ...
        *([f"# COPY ipset: {ipset_file} -> {ipset_export_path(prefix)}"] if ipset_file else []),
    ]
...
def build_keenetic_conf(
    *,
    ...
    udp_circular = (
        [f"--lua-desync={CIRCULAR_UDP}"] if flatten_export_strategies(udp_strategies) else []
    )
    udp_args = "\n ".join(
        [
    ...
    hdr = [
        ...
        *_keenetic_copy_header(tcp_strategies, udp_strategies, quic_strategies, prefix, ipset_file),
        *(
            [
...
def write_export_bundle(
    ...
    *,
    ...
    cores = flatten_export_strategies((tcp_strats or []) + (udp_strats or []) + (quic_strats or []))
    blob_dir = dest / "blobs"
