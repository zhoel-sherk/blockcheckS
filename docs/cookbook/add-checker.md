...
| [`test_runner.py`](../../src/blockchecks/service/test_runner.py) | `bs tcp`, `bs udp` (sync) |
| [`in_ns_workers.py`](../../src/blockchecks/service/in_ns_workers.py) | subprocess probe worker (`--mode curl\|udp`) |

For async batch tests, prefer calling the checker from the subprocess worker
(`python -m blockchecks.service.in_ns_workers --mode curl`, see
