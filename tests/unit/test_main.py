...
def test_campaign_domains_error():
    args = _args()
    with (
        patch("blockchecks.main.open_full_run_db", new=AsyncMock()) as db_open,
        patch("blockchecks.main.load_run_domains", return_value=([], "f", 4)),
    ):
...
def test_campaign_generate_error():
    ...
    with (
        ...
        patch("blockchecks.main.build_full_run_context", return_value=_ctx()),
        patch("blockchecks.main.generate_strategy_items", new=AsyncMock(return_value=8)),
    ):
        rc = asyncio.run(_run_full_campaign(args))
    assert rc == 8
...
def test_campaign_full_success():
    ...
    with (
        ...
        patch("blockchecks.main.run_http_phase", new=AsyncMock()),
        patch(
            "blockchecks.main.discover_voice_endpoint",
            new=AsyncMock(return_value=("1.2.3.4", 50004)),
        ),
        patch("blockchecks.main.run_quic_phase", new=AsyncMock()),
        patch("blockchecks.main.run_pairs_phase", new=AsyncMock()),
        patch("blockchecks.main.cleanup_runner", new=AsyncMock()) as cleanup,
...
def test_main_dispatches_and_returns_code():
    with (
        patch("blockchecks.engine.paths.apply_pycache_prefix"),
        patch("blockchecks.engine.paths.ensure_dirs"),
        patch("blockchecks.cli.user_config.load_user_config", return_value={}),
        patch("blockchecks.engine.paths.migrate_legacy_state_db"),
        patch("blockchecks.cli.user_config.finalize_store_args"),
        patch("blockchecks.main.validate_time_limit_args"),
        patch("blockchecks.main.ensure_system_deps_or_exit", return_value=0),
...
def test_main_deps_error_short_circuits():
    with (
        ...
        patch("blockchecks.cli.user_config.finalize_store_args"),
        patch("blockchecks.main.validate_time_limit_args"),
        patch("blockchecks.main.ensure_system_deps_or_exit", return_value=4),
