...
def test_default_paths_are_absolute():
    ...
    assert paths.DEFAULT_DB_PATH.parent == paths.DATA_DIR
...
def test_ensure_dirs_creates_tree(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(paths, "RUNTIME_LOGS_DIR", tmp_path / "data" / "logs")
    monkeypatch.setattr(paths, "USER_DATA_PRESETS_DIR", tmp_path / "data" / "presets")
