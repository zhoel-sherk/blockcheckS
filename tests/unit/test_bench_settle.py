...
def test_cmd_bench_settle_no_strategies():
    args = _args()
    with patch("blockchecks.cli.commands.bench_settle._load_strategies",
               return_value=[]):
...
def test_cmd_bench_settle_runs_grid(tmp_path):
    ...
    with patch("blockchecks.cli.commands.bench_settle._load_strategies",
        ...
        "blockchecks.cli.commands.bench_settle.AsyncTestRunner", return_value=runner
    ), patch("blockchecks.cli.commands.bench_settle._run_tcp_check",
             return_value={"success": True, "settle_ms": 100, "latency_ms": 50,
                           "http_code": 200}):
        rc = asyncio.run(cmd_bench_settle(args))
    assert rc == 0
...
def test_cmd_bench_settle_writes_profile(tmp_path):
    ...
    with patch("blockchecks.cli.commands.bench_settle._load_strategies",
        ...
        "blockchecks.cli.commands.bench_settle.AsyncTestRunner", return_value=runner
             ...
             return_value={"success": False, "settle_ms": 0, "latency_ms": 0,
                           "http_code": 0}), patch(
        "blockchecks.cli.commands.bench_settle.build_profile_from_rows",
        return_value=profile,
    ), patch("blockchecks.cli.commands.bench_settle.save_profile",
