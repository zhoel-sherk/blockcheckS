...
def test_test_single_success():
    ...
    with (
        ...
        patch.object(
            runner,
            "_run_check",
            return_value=MagicMock(success=True, latency_ms=10, http_status=200, error=None),
        ),
    ):
...
def test_test_config():
    ...
    with (
        ...
        patch.object(
            runner,
            "_run_check",
            return_value=MagicMock(success=True, latency_ms=5, http_status=200, error=None),
        ),
    ):
...
def test_test_sequential_breaks_on_deadline():
    ...
    with patch.object(runner, "test_single") as mock_single:
        mock_single.return_value = MagicMock(
            success=False, latency_ms=0, http_status=0, error=None, strategy="x"
        )
...
def test_test_sequential_udp():
    ...
    with patch.object(
        runner,
        "test_udp_config",
        return_value=MagicMock(success=False, latency_ms=0, error=None, strategy="c"),
    ):
