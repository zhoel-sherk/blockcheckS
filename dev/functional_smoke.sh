...
# pipefail + echo|grep -q SIGPIPE-fails on large DRY dumps; here-string is safe.
match_out() { grep -qiE "$1" <<<"${OUT}"; }
...
if match_out "Preflight|Triage |triage"; then report "bs preflight" 1; else report "bs preflight" 0; fi
...
if match_out "HTTP 200|passed"; then report "bs tcp single PASS" 1; else report "bs tcp single" 0; fi

# ── bs tcp --ns ──────────────────────────────────────────────
echo "--- bs tcp --ns ---" | tee -a "$LOG"
OUT=$(timeout 90 "$PY" "$ROOT/dev/run_tcp_ns_probe.py" discord.com 2>&1 || true)
echo "$OUT" | tee -a "$LOG" >/dev/null
if match_out "HTTP 200|passed"; then report "bs tcp --ns PASS" 1; else report "bs tcp --ns" 0; fi

# ── bs udp ───────────────────────────────────────────────────
echo "--- bs udp ---" | tee -a "$LOG"
OUT=$(timeout 60 sudo -n "$BS" udp -c configs/udp_voice__fake_r6.conf --ip 35.217.5.42 --port 50006 \
  --skip-deps-check 2>&1 || true)
if match_out "passed"; then report "bs udp voice probe" 1; else report "bs udp" 0; fi

# ── bs composite ─────────────────────────────────────────────
echo "--- bs composite ---" | tee -a "$LOG"
OUT=$(timeout 90 sudo -n "$BS" composite -c configs/composite_discord.conf -d discord.com \
  --skip-deps-check 2>&1 || true)
if match_out "HTTP 200|1/1 passed"; then report "bs composite" 1
else
  echo "$OUT" | tail -20 | tee -a "$LOG"
  report "bs composite" 0
fi
if match_out 'chmod 0777' && ! match_out 'warning'; then
...
if match_out "backend=lua_bridge" && match_out "passed"; then
...
if match_out "passed|TCP discord.com"; then report "bs pair tcp-only" 1; else report "bs pair tcp-only" 0; fi
...
if match_out "PASS|OK|settle"; then report "bs bench-settle" 1; else report "bs bench-settle" 0; fi
...
if match_out "Export configs|Run summary|TCP done"; then report "bs full quick" 1; else report "bs full quick" 0; fi
...
if match_out "backend=lua_bridge"; then report "bs scan --no-quarantine" 1; else report "bs scan --no-quarantine" 0; fi

# ── bs gc dry-run + harvest-batch ────────────────────────────
echo "--- bs gc --db-days ---" | tee -a "$LOG"
OUT=$(timeout 30 "$BS" gc --db-days 14 --db "/tmp/fs_pair_$TS.db" 2>&1 || true)
if match_out "DRY|re-run with --apply|gc db|SKIP|deletes"; then
...
if match_out "harvest →|candidates:|no candidates"; then
...
if match_out "No active|Stopped"; then report "bs stop (no-op)" 1; else report "bs stop" 0; fi

# ── bc-nfconf export ─────────────────────────────────────────
echo "--- bc-nfconf export ---" | tee -a "$LOG"
OUT=$(timeout 60 "${ROOT}/.venv/bin/bc-nfconf" --db "/tmp/fs_pair_$TS.db" -d discord.com \
  --out-dir "/tmp/fs_nfconf_$TS" --limit 3 --no-common-only --allow-stock-fallback 2>&1 || true)
if match_out "keenetic|nfqws2_" || ls /tmp/fs_nfconf_"$TS"/nfqws2_*.conf >/dev/null 2>&1; then
  report "bc-nfconf export" 1
else
  report "bc-nfconf export" 0
fi

# ── bs data-block export ────────────────────────────────────
echo "--- bs data-block export ---" | tee -a "$LOG"
OUT=$(timeout 30 "$BS" data-block --out "/tmp/fs_datablock_$TS" 2>&1 || true)
if match_out "export complete"; then report "bs data-block export" 1; else report "bs data-block export" 0; fi
...
if match_out "Wrote|shortlist"; then
  OUT2=$(timeout 60 "$PY" -m blockchecks.shortlist_import -i "$SL" --out-dir "$SL_OUT" --prefix fs 2>&1 || true)
  if grep -qiE "Imported" <<<"${OUT2}"; then report "shortlist round-trip" 1; else report "shortlist round-trip" 0; fi
else
